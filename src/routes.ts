...
export const glideMQRoutes: FastifyPluginAsync<GlideMQRoutesOptions> = async (
  fastify,
  options,
) => {
  ...
  fastify.addHook("preHandler", async (request, reply) => {
    ...
    if (url.includes("/broadcast/")) {
      if (allowedQueues && !allowedQueues.includes(name)) {
        return reply
          .code(404)
          .send({ error: "Queue not found or not accessible" });
      }
      return;
    }
    if (url.endsWith("/produce")) {
      ...
      if (
        (allowedProducers && !allowedProducers.includes(name)) ||
        !registry.hasProducer(name)
      ) {
        return reply
          .code(404)
          .send({ error: "Producer not found or not accessible" });
      }
      return;
    }
    ...
    const registry = getRegistry();
    if (
      (allowedQueues && !allowedQueues.includes(name)) ||
      !registry.has(name)
    ) {
      return reply
        .code(404)
  ...
  }>("/usage/summary", async (request, reply) => {
    try {
      ...
      if (window && windowMs && window !== windowMs) {
        return reply.code(400).send({
          error: "window and windowMs must match when both are provided",
        });
      }
  ...
  fastify.post("/flows", async (request, reply) => {
    try {
      ...
      if ((!!body.flow && !!body.dag) || (!body.flow && !body.dag)) {
        return reply
          .code(400)
          .send({ error: "Body must include exactly one of: flow, dag" });
      }
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .code(
            message.includes("not accessible") ? 404 : flowErrorStatus(message),
          )
          .send({ error: message });
      }
    },
    ...
    async (request, reply) => {
      ...
      if (schemas) {
        ...
        if (!result.success) {
          ...
          return reply
            .code(400)
            .send({ error: "Validation failed", details: issues });
        }
      ...
      if (!body?.name || typeof body.name !== "string") {
        return reply
          .code(400)
          .send({ error: "Validation failed", details: ["name: Required"] });
      }
  ...
  fastify.get<{
    ...
    if (!VALID_JOB_TYPES.includes(typeParam as any)) {
      return reply.code(400).send({
        ...
        details: [`type: must be one of ${VALID_JOB_TYPES.join(", ")}`],
      });
    }
    ...
    if (isNaN(start) || isNaN(end)) {
      return reply.code(400).send({
        error: "Validation failed",
        details: ["start and end must be numbers"],
      });
    }

    const excludeData =
      request.query.excludeData === "true" || request.query.excludeData === "1";
    const jobs = excludeData
      ? await (queue as any).getJobs(typeParam, start, end, {
          excludeData: true,
        })
  ...
  fastify.post<{ Params: { name: string; id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (
        ...
        return reply.code(400).send({
          error: "Validation failed",
          details: ["delay must be a non-negative integer"],
        });
      }
  ...
  fastify.get<{
    Params: { name: string };
    Querystring: { type?: string; start?: string; end?: string };
  }>("/:name/metrics", async (request, reply) => {
    const { name } = request.params;
    const registry = getRegistry();
    const { queue } = registry.get(name);
    ...
    if (!VALID_METRICS_TYPES.includes(typeParam as any)) {
      return reply.code(400).send({
        error: "Validation failed",
        details: [`type: must be one of ${VALID_METRICS_TYPES.join(", ")}`],
      });
    }
  ...
  fastify.post<{ Params: { name: string } }>(
    ...
    async (request, reply) => {
      ...
      if (count !== undefined && (!Number.isInteger(count) || count < 1)) {
        return reply.code(400).send({
          error: "Validation failed",
          details: ["count must be a positive integer"],
        });
      }
  ...
  fastify.delete<{
    Params: { name: string };
    Querystring: { grace?: string; limit?: string; type?: string };
    ...
    if (!VALID_CLEAN_TYPES.includes(typeParam as any)) {
      return reply.code(400).send({
        error: "Validation failed",
        details: [`type: must be one of ${VALID_CLEAN_TYPES.join(", ")}`],
      });
    }
    ...
    if (isNaN(grace) || isNaN(limit) || grace < 0 || limit < 1) {
      return reply.code(400).send({
        error: "Validation failed",
        details: ["grace must be >= 0 and limit must be >= 1"],
      });
    }
  ...
  fastify.get<{ Params: { name: string; schedulerName: string } }>(
  ...
  fastify.put<{ Params: { name: string; schedulerName: string } }>(
    "/:name/schedulers/:schedulerName",
    async (request, reply) => {
      const { name, schedulerName } = request.params;
  ...
  fastify.delete<{ Params: { name: string; schedulerName: string } }>(
    "/:name/schedulers/:schedulerName",
    async (request, reply) => {
      const { name, schedulerName } = request.params;
  ...
  fastify.get<{
    ...
    Querystring: { lastId?: string };
  }>("/:name/jobs/:id/stream", async (request, reply) => {
    ...
    let lastId =
      ...
      (request.query.lastId as string) ||
      undefined;
    let closed = false;
    ...
    try {
      while (!closed) {
        const entries = await (queue as any).readStream(jobId, {
          lastId,
          count: 100,
        });
        ...
        if (state === "completed" || state === "failed") {
          const trailing = await (queue as any).readStream(jobId, {
            lastId,
            count: 100,
          });
      ...
      try {
        ...
        try {
          const id = await broadcast.publish(
            body.subject,
            body.data ?? null,
            pickOpts(body.opts ?? {}) as any,
          );
          return reply
            .code(id ? 201 : 200)
            .send(id ? { id, subject: body.subject } : { skipped: true });
        } finally {
  ...
  fastify.get<{
    Params: { name: string };
    Querystring: { subscription?: string; subjects?: string };
  }>("/broadcast/:name/events", async (request, reply) => {
    const { name } = request.params;
    const subscription = request.query.subscription;
    if (!subscription) {
      return reply
        .code(400)
        .send({ error: "Missing required query param: subscription" });
    }
